...
class SingleRowVRegBounds : public VRegDataBounds {
 public:
  ...
  FailureOr<TypedValue<VectorType>> getVectorMask(
    ...
    const auto getI32VregConstant = [&](const int64_t v) {
      return builder.create<arith::ConstantOp>(
          builder.getUnknownLoc(), i32_vreg,
    ...
    const Value iota =
        builder.create<tpu::IotaOp>(builder.getUnknownLoc(), i32_vreg, nullptr);
    return cast<TypedValue<VectorType>>(
        builder
            .create<arith::AndIOp>(builder.getUnknownLoc(),
                                   builder.create<arith::CmpIOp>(
                                       builder.getUnknownLoc(),
                                       arith::CmpIPredicate::sge, iota, start),
                                   builder.create<arith::CmpIOp>(
                                       builder.getUnknownLoc(),
                                       arith::CmpIPredicate::slt, iota, end))
            .getResult());
  }
...
class TiledRectangularVregBounds : public VRegDataBounds {
 public:
  TiledRectangularVregBounds(const VectorLayout& layout,
    ...
    CHECK(
        llvm::all_of(llvm::zip(start_offsets_, layout_.tiling()), [](auto tup) {
          auto [o, t] = tup;
          return 0 <= o && o < t;
        }));
    CHECK(llvm::all_of(llvm::zip(end_offsets_, layout_.tiling()), [](auto tup) {
      auto [o, t] = tup;
      return 0 <= o && o <= t;
    }));
  }
  ...
  DenseBoolArrayAttr getSublaneMask(
    ...
    return DenseBoolArrayAttr::get(mlir_ctxt, mask);
  }
...
std::unique_ptr<VRegDataBounds> VectorLayout::tileDataBounds(
    MLIRContext* mlir_ctxt, const ArrayRef<int64_t> full_shape,
  ...
  if (!hasNaturalTopology(target_shape)) {
    ...
    // TODO(tlongeri): Original code uses tiling[-1] instead of tiling[1], even
    // though tiling is supposed to be a tuple[int, int]?
  ...
  if (!offsets_[0].has_value()) {
    if (!allow_replicated[0]) {
      emitError(UnknownLoc::get(mlir_ctxt), "Unexpected replicated offset");
      return nullptr;
    }
