...
FailureOr<BlockArgument> appendConstant(RewriteContext &ctxt,
                                        DenseElementsAttr value) {
  MLIRContext *mlir_ctxt = ctxt.func.getContext();
  Block &entry_block = ctxt.func.getBody().front();
  auto value_ty = cast<VectorType>(value.getType());
  if (auto integer_ty = dyn_cast<IntegerType>(value_ty.getElementType())) {
    if (integer_ty.getWidth() != 32) {
      return ctxt.func.emitOpError("Only 32-bit constants supported");
    }
  } else if (dyn_cast<Float32Type>(value_ty.getElementType()) == nullptr) {
    return ctxt.func.emitOpError("Only 32-bit constants supported");
  }
  ...
  const auto new_func_ty =
      FunctionType::get(mlir_ctxt, new_arg_tys, func_ty.getResults());
  ctxt.func.setFunctionType(new_func_ty);
  // Adjust the constants attribute.
  if (auto prev_cst = ctxt.func->getAttrOfType<ArrayAttr>("vector_constants")) {
    SmallVector<Attribute> vector_constants(prev_cst.getValue());
    vector_constants.push_back(value);
    ctxt.func->setAttr(
        "vector_constants",
        ArrayAttr::get(ctxt.func.getContext(), vector_constants));
  }
  // Adjust window params for the extra operand.
  if (auto window_params =
          ctxt.func->getAttrOfType<ArrayAttr>("window_params")) {
    // TODO(tlongeri): Why is this required?
    const auto iteration_bounds =
        ctxt.func->getAttrOfType<DenseI64ArrayAttr>("iteration_bounds");
    CHECK(iteration_bounds);
    const int64_t iteration_rank = iteration_bounds.getSize();
    const SmallVector<AffineExpr> zeros(
        iteration_rank, getAffineConstantExpr(0, ctxt.func.getContext()));
    const auto transform_indices =
        AffineMap::get(iteration_rank, 0, zeros, ctxt.func.getContext());
    const auto new_param = DictionaryAttr::get(
        ctxt.func.getContext(),
        NamedAttribute(
            StringAttr::get(ctxt.func.getContext(), "transform_indices"),
    ...
    ctxt.func->setAttr("window_params", ArrayAttr::get(ctxt.func.getContext(),
...
LogicalResult arith_constant_rule(RewriteContext &ctxt, Operation &op,
                                  const ArrayRef<Layout> layouts_in,
                                  const ArrayRef<Layout> layouts_out) {
  ...
  if (vty) {
    ...
    if (value.isSplat()) {
      ...
      const auto tile = ctxt.builder.create<arith::ConstantOp>(
          op.getLoc(), target_vty, new_value);
      const xla::Array<Value> tiles(
          layout_out.tileArrayShape(vty.getShape(), ctxt.target_shape),
          tile->getResult(0));
      op.replaceAllUsesWith(assemble(ctxt, vty, layout_out, std::move(tiles)));
      op.erase();
      return success();
    }
    // !value.isSplat()
    ...
    FAILUREOR_ASSIGN_OR_RETURN(const BlockArgument ref,
                               appendConstant(ctxt, value));
    auto load_op = ctxt.builder.create<vector::LoadOp>(
        op.getLoc(), vty, ref,
        SmallVector<Value>(vty.getRank(),
                           IdxCst(0, ctxt.builder, op.getLoc())));
    op.replaceAllUsesWith(ArrayRef<Value>{load_op.getResult()});
    op.erase();
    const SmallVector<Layout> vector_load_in_layouts(vty.getRank() + 1);
    return vector_load_rule(ctxt, *load_op, vector_load_in_layouts,
                            {VectorLayout(/*bitwidth=*/32, /*offsets=*/{0, 0},
                                          /*tiling=*/ctxt.target_shape)});
  }
